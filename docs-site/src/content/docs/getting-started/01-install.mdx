---
title: Install OMem
description: Install the omem CLI on macOS with a one-line curl script, and confirm it runs. The first of four steps to get OMem working end to end.
---
...
OMem ships as a single compiled binary. You won't `pip install` it, clone a repo, or manage a virtualenv — the install script downloads one executable and that's the whole install. Upgrades come through the same script.
</Aside>
