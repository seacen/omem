...
Plugins exist to offer *different trade-offs* (speed vs. recall, local vs. cloud), not to upgrade from a deliberately weak baseline. The `brew install omem` default has to be something you can trust every day — see [design principle P6](/concepts/06-design-principles/). `qmd` isn't "the good version"; it's "the different-trade-off version".
</Aside>
